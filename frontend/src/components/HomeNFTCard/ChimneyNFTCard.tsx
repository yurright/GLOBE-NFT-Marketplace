...
import Modal from "../BuyModal/ChimneyBuyModal";
...
const ChimneyNFTCard: FC<NFTCardProps> = ({ image, name, tokenId }) => {
  ...
  const onClickModal = async () => {
    if (!account || !mintChimneyNFTContract || !salePartsNFTContract) {
      alert("로그인하세요");
      return;
    }
  ...
  return (
    <>
      ...
      {isOpen && <Modal setIsOpen={setIsOpen} tokenId={tokenId} />}
    </>
  );
};

export default ChimneyNFTCard;
