...
import Modal from "../BuyModal/WindowBuyModal";
import WindowBuyModal from "../BuyModal/WindowBuyModal";
...
const WindowNFTCard: FC<NFTCardProps> = ({ image, name, tokenId }) => {
  ...
  const onClickModal = async () => {
    if (!account || !mintWindowNFTContract || !salePartsNFTContract) {
      alert("로그인하세요");
      return;
    }
  ...
  return (
    <>
      ...
      {isOpen && <WindowBuyModal setIsOpen={setIsOpen} tokenId={tokenId} />}
    </>
  );
};

export default WindowNFTCard;
